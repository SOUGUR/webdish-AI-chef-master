  ...
  return (
      <nav className='xl:hidden'>
        <div className='flex gap-[1rem] items-center'>
          {localStorage.getItem('theme') === 'light' || theme === 'light' ? (
              <button title="Switch to Dark Mode" onClick={handleThemeChange}
                      className='flex items-center justify-center rounded-md px-2 py-2 bg-[#eaeaea] text-sm font-medium'>
                <MdDarkMode size={20}/>
              </button>
          ) : (
        ...
        >
          <div onClick={() => {
            setOpenMenu(false)
          }} className='text-4xl absolute z-30 left-4 top-14 cursor-pointer text-black'>
            <IoMdClose/>
          </div>
          ...
          <ul className='h-[100dvh] flex flex-col justify-center items-center gap-y-6  font-primary first-letter:first font-medium text-2xl '>
            ...
            {user && (<div className='flex-col flex items-center  md:flex-row'>
              ...
              <button onClick={handleClick}
                      className='border text-base my-4 border-amber-500 px-2  rounded-full mx-2 '>Logout
              </button>
            </div>)}
            ...
            {!user && (
                <>
                  <li key='login'
                      className=' text-lg  font-primary px-4   transition-all duration-300 border border-yellow-800 rounded-xl hover:bg-[#ff910032]  py-2 '>
