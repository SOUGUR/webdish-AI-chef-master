from flask import Flask, request, jsonify, url_for, redirect, session, render_template, send_file
...
def get_video(video_id):
    ...
    return send_file(BytesIO(video.read()), mimetype='video/mp4')
...
def fetch_dishes():
    ...
    #     col = db['Dish']
    #     print("abcd")
    #     dishes = col.find()  # {}, {'dish_name': 1, 'description': 1})
    #     dishes_list = list(dishes)
    #     return jsonify({'data': dishes_list})
    # except Exception as e:
    #     return jsonify({'error': str(e)}), 500
    dishes = [
        {'name': 'Pasta', 'description': 'Delicious pasta with tomato sauce'},
        {'name': 'Pizza', 'description': 'Cheesy pizza with pepperoni'}
    ]
...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        file.save(file_path)
        video_url = upload_to_firebase(file_path, filename)
        os.remove(file_path)
...
def list_videos():
    ...
    video_urls = [blob.public_url for blob in blobs if blob.name.split('.')[-1].lower() in ALLOWED_EXTENSIONS]
